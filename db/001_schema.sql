-- Room planner schema
--
-- UNITS: every physical dimension is an INTEGER in MILLIMETRES, suffixed _mm.
-- Angles are integers in tenths of a degree, suffixed _ddeg (3600 = 360 deg).
-- Money is an integer in cents, suffixed _cents.
-- Never store floats for geometry: 0.1 + 0.2 problems show up as furniture
-- that "almost" fits. Convert to cm at the UI boundary only.
--
-- ORIGIN: room coordinates are 2D floor-plan coordinates in mm, origin at the
-- room's bottom-left bounding corner, +x right, +y up (mathematical, not screen).
-- z is height above the finished floor.
